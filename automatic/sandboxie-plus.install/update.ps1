...
function global:au_GetLatest {
    ...
    $version = $matches['plus']
    if ($matches['plus_letter']) {
        $suffix = '.' + [string]([byte][char]$matches['plus_letter'] - [byte][char]'a' + 1)
        $version = $version.Substring(0, $version.Length - 1) + $suffix
    }
